...
const findMaxMinuteAsleep = (guardSchedule) => {
  ...
  return mins.indexOf(Math.max(...mins));
}
...
const findMaxAsleep = schedule => {
  ...
  for(let guard in byGuard) {
    const asleep = calcTotalAsleep(byGuard[guard])
    if (asleep > maxAsleep) {
      maxGuard = guard;
      maxAsleep = asleep;
    }
  ...
  return findMaxMinuteAsleep(byGuard[maxGuard]) * maxGuard;
}
...
assert(findMaxAsleep(test1) === 240);
